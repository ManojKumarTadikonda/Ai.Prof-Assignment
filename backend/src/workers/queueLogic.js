...
export async function processQueueOnce() {
  for (const h of await Hospital.find({ status: "ACTIVE" })) {
    ...
    const tasks = await OutreachTask.find({
      ...
      .limit(Math.max(0, h.outboundCapacity - active));
    for (const t of tasks) {
      const p = await Patient.findById(t.patientId),
        c = await Campaign.findById(t.campaignId);
      if (!p || !c) continue;
      ...
      try {
        await sendOutreachEmail({
          to: p.email,
          patientName: p.name,
          link: `${env.appUrl}/patient/followup/${token}`,
          hospitalName: h.name,
        });
        t.status = "SCHEDULED";
        t.attempts++;
        ...
        t.attempts++;
        t.status =
          t.attempts >= h.retry.maxAttempts
            ? "MANUAL_FOLLOW_UP"
            : "RETRY_SCHEDULED";
        t.nextAttemptAt = new Date(Date.now() + h.retry.backoffMinutes * 60000);
